...
export var BodyView = (function () {
    ...
    BodyView.prototype.addToStage = function () {
        this._callout.container.appendChild(this._bodyEl);
        this.hide();
        this._bodyEl.style.transform = 'scale(1)';
        var rect = Rect.fromBounds(relBounds(this._callout.container, this._bodyEl));
    ...
    BodyView.prototype.removeFromStage = function () {
    ...
    BodyView.prototype.calculateLayout = function (currentRect) {
        ...
        if (0 > targetRect.x1)
            targetRect.x1 = 0;
        if (targetRect.x2 > containerRect.x2)
            targetRect.x1 = containerRect.x2 - targetRect.width;
        if (0 > targetRect.y1)
            targetRect.y1 = 0;
        if (targetRect.y2 > containerRect.y2)
            targetRect.y2 = containerRect.y2 - targetRect.height;
    ...
    BodyView.prototype.handlDragEvent = function (evt) {
        ...
        this.updateLayout();
        setTimeout(function () {
            var rect = Rect.fromBounds(relBounds(_this._callout.container, _this._bodyEl));
            _this._layoutData.fullSize = rect.dimension;
